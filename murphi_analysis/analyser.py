...
class Ruleset(object):
    ...
    def sparse_rulesets(self, aux_invs, abs_type, boundary_K, logfile, home_flag, print_usedinvs_to_file=False):
        ...
        for params, rules_str in rulesets:
            ...
            for each_rule in rules:
                ...
                #print('rulename:{}, guards_obj:{}, actions_obj:{}'.format(rulename, guards_obj, actions_obj ))
                ...
                #print('len(useful_invs):{}'.format(len(useful_invs)))
                dict_inv2index = refiner.dict_inv2index
                #print('dict_inv2index:{}'.format(len(dict_inv2index)))
                #print('guard_obj:{}'.format(guards_obj.normal_guards))
                ...
                abstracter.abstract()
                used_inv_string_list_filfered = []
                #print('type:{}'.format(type(abstracter.used_inv_string_list)))
                #print('abstracter.used_inv_string_list:{}'.format(abstracter.used_inv_string_list))
                for used_inv_string in abstracter.used_inv_string_list:
                    pre, cond = used_inv_string.split('->')[0].strip(), used_inv_string.split('->')[1].strip()
                    if '&' not in used_inv_string:
                        if 'NODE' in pre and 'NODE' in cond:
                            preInside = re.findall(r'\[NODE\_\d', pre)[0]
                            condInside = re.findall(r'\[NODE\_\d', cond)[0]
                            if preInside == condInside:
                                continue
                        elif '[' not in pre:
                            continue
                        used_inv_string_list_filfered.append(used_inv_string)
                    else:
                        pre1, pre2 = pre.split('&')[0].strip(), pre.split('&')[1].strip()
                        if 'NODE' in pre1 and 'NODE' in pre2 and 'NODE' in cond:
                            preInside1 = re.findall(r'\[NODE\_\d', pre1)[0]
                            preInside2 = re.findall(r'\[NODE\_\d', pre2)[0]
                            condInside = re.findall(r'\[NODE\_\d', cond)[0]
                            if preInside1 == condInside and preInside2 == condInside:
                                continue
                        elif '[' not in pre1 and '[' not in pre2:
                            continue
                        used_inv_string_list_filfered.append(used_inv_string)
                abstracter.used_inv_string_list = used_inv_string_list_filfered
                ...
                #print('self.used_inv_string_set:{}'.format(self.used_inv_string_set))
                #print('len(self.used_inv_string_set):{}'.format(len(self.used_inv_string_set)))
    ...
    def write_usedinv_to_file(self, string_list, rulename):
        ...
        fout = '{}/{}/middleDocument.txt'.format(self.data_dir, self.protocol_name)
        with open(fout, 'a') as f:
            if string_list:
                tempString = '{'
                for i in range(len(string_list)):
                    pre, cond = string_list[i].split('->')[0].strip(), string_list[i].split('->')[1].strip()
                    tempcond = cond
                    pre = re.sub(r'[A-Z]*\_\d','i', pre)
                    if re.findall('[A-Z]*\_\d', cond):
                        for j in range(0, 2):
                            tempString += 'strength('
                            cond = re.sub(r'[A-Z]*\_\d', '%s' % (j+1), tempcond)
                            if i == len(string_list)-1 and j == 1:
                                tempString = tempString + pre + ' -> ' + cond + ')'
                            else:
                                tempString = tempString + pre + ' -> ' + cond + '), '
                    else:
                        tempString += 'strength('
...
class Reiner(object):
    ...
    def find_useful_invs(self, boundary_K, guard=set()):
        ...
        for k in range(boundary_K):
            ...
            #print('iter_cons:{}'.format(iter_cons))
        ...
        #print('used_invset:{}'.format(used_invset))
        self.evaluate(used_invset)
        #print('used_inv_index:{}'.format(self.used_inv_index))
    ...
    def find_useful_invs_iter(self, guard):
        ...
        for i, (pre, cond) in enumerate(self.aux_invs, 1):
            if set(guard) & pre == pre:
                ...
                #print('setguard:{}'.format(set(guard)))
                #print('pre:{}, cond:{}'.format(pre, cond))
                # record used aux invs
                if cond not in set(guard):
                    self.used_inv_index.add(i)
        #print('useful_cond:{}'.format(useful_cond))
        return useful_cond
    ...
    def evaluate(self, used_invset):
        if not used_invset:
            return
        #print('used_invset:{}'.format(used_invset))
        #print("\n[Candidate association rules used by {}]: {} :{}".format(self.rulename, len(used_invset), used_invset))
...
class Abstractor(object):
    ...
    def rep_global(self, aux_inv_list, action_obj, abs_para_list):
        ...
        #aux_inv_list:使用过的 辅助不变式的前缀
        #[({'n[NODE_1].st = E'}, 'n[NODE_2].st != C'), ({'n[NODE_1].st = E'}, 'x = false'), 
        #({'n[NODE_1].st = E'}, 'n[NODE_2].st != E'), ({'n[NODE_1].st = E'}, 'n[NODE_1].st != C'), 
        #({'n[NODE_1].st = E'}, 'n[NODE_1].data = auxDATA'), ({'n[NODE_1].st = E'}, 'n[NODE_1].st != T'), 
        #({'n[NODE_1].st = E'}, 'n[NODE_1].st != I')]
        # ({'n[NODE_1].st = C'}, 'n[NODE_1].st != E')
        for pre, cond in aux_inv_list:
            ...
            #print('rep_dict:{},rep_dict_to_string:{}'.format(rep_dict, rep_dict_to_string))

        is_repl = False
        for stmt in list(filter(lambda x: re.findall(':=', x), action_obj.mainfield.content)):
            #print('list:{}'.format(list(filter(lambda x: re.findall(':=', x), action_obj.mainfield.content))))
            ...
            for abs_para in abs_para_list:
                if re.findall(r'%s' % abs_para, assign):
                    if assign in rep_dict:
                        ...
                        #print('assign:{}'.format(assign))
                        #print('rep_dict[assign]:{}'.format(rep_dict[assign]))
                        # is_repl = True
                        print('stmt:{},assign:{},rep_dict[assign]:{}'.format(stmt, assign, rep_dict[assign]))
                        ...
                        #print('self.used_inv_string_list:{}'.format(self.used_inv_string_list))
                        self.used_inv_string_list.append(rep_dict_to_string[assign])
                        #print('rep_dict_to_string:{}'.format(rep_dict_to_string))
                        #print('self.used_inv_string_listAfter:{}'.format(self.used_inv_string_list))
                        #print('assign:{}, rep_dict_to_string[assign]:{}'.format(assign, rep_dict_to_string[assign]))
        ...
        #print('rep_index:{}'.format(rep_index))
        #print('{}'.format(self.action_obj.mainfield.content))
        #print('{}'.format(self.action_obj.mainfield.para_dict))
        return rep_index

    def abstract(self):
        main_para = self.guard_obj.mainfield.para_dict
        #print('main_para:{}'.format(main_para))
        cnt = 0
    ...
    def asbtract_obj(self, origin_obj, abs_para_list):
        for abs_para in abs_para_list:
            #print('origincontent:{}'.format(origin_obj.mainfield.content))
            origin_obj.mainfield.content = self.abstract_list(abs_para, origin_obj.mainfield.content)
            #print('aftercontent:{}'.format(origin_obj.mainfield.content))
            index = 0
            ...
            while index < len(origin_obj.existfield) and origin_obj.existfield:
                ...
                index += 1
        #print('[origin_obj]:{}'.format([origin_obj][0]))
        return [origin_obj]
    ...
    def abstract_1_para(self, abs_para_list: list = None):
        ...
        #print('self.aux_invs:{}'.format(self.aux_invs))

        rep_index = self.rep_global(self.aux_invs, self.action_obj, abs_para_list)
        #print('rep_index:{}'.format(rep_index))
        #print('self.action_obj:{}'.format(self.action_obj.normal_guards))
        #print('self.guard_obj:{}'.format(self.guard_obj.normal_guards))
        #print('abs_para_list:{}'.format(abs_para_list))
        abs_action_obj = self.asbtract_obj(self.action_obj, abs_para_list)[0]
        #print('abs_action_obj:{}'.format(abs_action_obj.normal_guards))
        abs_guard_obj = self.asbtract_obj(self.guard_obj, abs_para_list)[0]
        #print('abs_guard_obj:{}'.format(abs_guard_obj.normal_guards))
        #print('self.aux_invs:{}'.format(self.aux_invs))
        abs_inv = self.abstract_inv(self.aux_invs, abs_para_list)
        #print('abs_inv:{}'.format(abs_inv))
        ...
        if check_empty(abs_action_obj):
            ...
            #print('abs_rule_name:{}',format(abs_rule_name))
    ...
    def print_abs_rule(self, abs_guard_obj, abs_action_obj, abs_inv, abs_para_list, abs_rulename):
        ...
        print('para_dcit:{}'.format(para_dict))
        try:
            ...
            #print('main_list:{}'.format(abs_guard_obj.mainfield.para_dict))
            print_string = ""
            ...
            #print('guard_string:{}'.format(guard_string))
            inv_string = self.print_inv(abs_inv, para_dict)
            #print('inv_string:{}'.format(inv_string))
            ...
            if guard_string and inv_string:
                ...
                print_string += '\n\tTrue'
            #print('print_string:{}'.format(print_string))
            print_string += '\n==>\nbegin'
            ...
            print('action:{}'.format(self.print_action(
                pop_key_from_field_list(abs_action_obj, set(abs_para_list) | set(main_list)))))
            #print('print_string:{}'.format(print_string))
...
class Protocol(object):
    ...
    def refine_abstract(self, aux_invs, abs_type, print_usedinvs_to_file=False, boundary_K=1):
        ...
        #print('abs_type:{}'.format(abs_type)),abs_type=NODE
